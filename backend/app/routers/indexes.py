...
# TODO : Changed the return type from dict to list
@router.get("/list_firestore_databases")
async def list_firestore_databases(index_manager=Depends(get_index_manager)) -> list:
    active_db_name = index_manager.firestore_db_name
    logger.info(f"active_db_name: {active_db_name}")
    ...
    try:
        for database in client.list_databases(parent=parent).databases:
            logger.info(f"In loop: {database.name}")
