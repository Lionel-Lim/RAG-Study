...
def deploy_index(
    ...
    vs_deployed_index = aiplatform.MatchingEngineIndexEndpoint(
        index_endpoint_name=index_endpoints[0][0], credentials=google_credential
    )
    print(
        f"Vector Search index {vs_index.display_name} "
...
def get_existing_index_and_endpoint(
    vector_index_name: str,
    ...
    endpoint_list = aiplatform.MatchingEngineIndexEndpoint.list(
        filter=f"display_name={index_endpoint_name}", credentials=google_credential
    )
    if endpoint_list:
...
def get_or_create_existing_index(
    ...
    vs_index, vs_endpoint = get_existing_index_and_endpoint(
        vector_index_name, index_endpoint_name, google_credential
    )
    ...
    if not vs_index:
        vs_index = create_index(
            vector_index_name, approximate_neighbors_count, google_credential
        )
    if not vs_endpoint:
        vs_endpoint = create_endpoint(index_endpoint_name, google_credential)
    deploy_index(vs_index, vs_endpoint, vector_index_name, google_credential)
