...
class DocAIParser:
    """
    Class for interfacing with Document AI API.
    """

    def __init__(
        self, project_id: str, location: str, processor_name: str, gcs_output_path: str
    ):
    ...
    ) -> tuple[list[Document], list["DocAIParsingResults"]]:
        """
        Batch parse a list of blobs using Document AI.

        Args:
            blobs: List of Blob objects (GCS Blobs) to parse.
            chunk_size: Chunk size for Document AI Processing.
            include_ancestor_headings: Whether to include ancestor headings in the parsed text.
            timeout_sec: Timeout in seconds.
            check_in_interval_sec: Interval in seconds to check in on the parsing job.

        Returns:
            Tuple of list of parsed documents and list of parsing results.
        """
        try:
    ...
    def _wait_for_operations(
        self, operations: list, timeout_sec: int, check_in_interval_sec: int
    ):
        """
        ...
        for operation in operations:
            if operation.exception():
                raise KeyError(f"Operation failed with error: {operation.exception()}")

    def _get_results(self, operations: list) -> list["DocAIParsingResults"]:
        results = []
        for operation in operations:
    ...
    def _parse_from_results(self, results: list["DocAIParsingResults"]):
        documents = []
        ...
        for result in results:
            ...
            if not result.parsed_path:
                print(
                    "Warning: Parsed path is empty."
                    f"{result.source_path} will be skipped."
                )
                continue
            ...
            try:
                ...
                print(f"Warning: Invalid parsed path: {result.source_path}")
                continue
